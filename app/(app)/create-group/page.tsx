...
export default function CreateGroupPage() {
  ...
  const [tournamentWinner,setTournamentWinner]= useState(100);
  ...
  if (passkey) {
    return (
      <div className="max-w-lg mx-auto space-y-5 px-4 py-6">
        <div className="rounded-2xl p-6 text-center space-y-4"
          style={{ background: "rgba(255,255,255,0.9)", border: "1px solid rgba(0,255,136,0.3)", boxShadow: "0 4px 24px rgba(0,255,136,0.1)" }}>
          ...
          <div>
            <h2 className="font-display text-3xl uppercase font-black" style={{ color: "#0F172A" }}>{groupName}</h2>
            <p className="text-sm mt-1" style={{ color: "#64748b" }}>Group created! Share the passkey with your squad.</p>
          </div>
          ...
          <div className="rounded-2xl p-5"
            ...
            <div className="text-xs mt-2" style={{ color: "#94a3b8" }}>Members pay $2 to join the leaderboard</div>
          </div>

          <div className="rounded-xl px-4 py-2.5 font-mono text-xs break-all text-left"
  ...
  // ── Step indicator ─────────────────────────────────────────────────────────
  const steps = [
    { n: 1, label: "Group Setup" },
    { n: 2, label: "Buy-In & Prizes" },
    { n: 3, label: "Scoring Rules" },
  ];
  ...
  return (
    <div className="max-w-lg mx-auto space-y-5 px-4 py-6">
      ...
      <div className="flex gap-2">
        {steps.map(s => (
          <button key={s.n} onClick={() => { if (s.n < step || step === s.n) return; }}
      ...
      {step === 1 && (
        <div className="space-y-4">
                  ...
                  <div className="absolute top-full left-0 right-0 mt-1 rounded-xl border overflow-hidden z-20"
                    ...
                    {FEATURED_MATCHES.map(m => (
                      <button key={m.id} onClick={() => { setSelectedMatch(m.id); setShowMatchPicker(false); }}
                        ...
                        style={{ color: selectedMatch === m.id ? "#0891B2" : "#475569", borderBottom: "1px solid #f1f5f9" }}>
          ...
          <button onClick={() => { if (!groupName.trim()) { setError("Group name is required"); return; } setError(null); setStep(2); }}
            className="w-full py-3 rounded-xl font-bold text-sm uppercase tracking-wider flex items-center justify-center gap-2"
            style={{ background: "linear-gradient(135deg, #00FF88, #00D4FF)", color: "#0B141B" }}>
        ...
        <div className="space-y-4">
          <div className="rounded-2xl p-5 space-y-4"
            ...
            <p className="text-xs" style={{ color: "#94a3b8" }}>
              Buy-in is for your records only — Cup Clash tracks who paid but doesn&apos;t handle the money.
            </p>
            ...
            <div>
              ...
              <div className="space-y-2">
                {[
                  { label: "🥇 1st place", key: "first",  val: payoutFirst,  set: setPayoutFirst,  prize: prize1, color: "#d97706" },
                  { label: "🥈 2nd place", key: "second", val: payoutSecond, set: setPayoutSecond, prize: prize2, color: "#64748b" },
                  { label: "🥉 3rd place", key: "third",  val: payoutThird,  set: setPayoutThird,  prize: prize3, color: "#b45309" },
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => { if (totalPct !== 100) { setError("Payouts must add up to 100%"); return; } setError(null); setStep(3); }}
      ...
      {step === 3 && (
        <div className="space-y-4">
          <div className="rounded-2xl p-5 space-y-4"
            ...
            {[
              { label: "Correct outcome (win/draw/loss)", val: correctOutcome, set: setCorrectOutcome, desc: "Right result, wrong score" },
              { label: "Exact score bonus",               val: exactScore,     set: setExactScore,     desc: "Perfect score guess" },
              { label: "Knockout advancement",            val: koAdvancement,  set: setKoAdvancement,  desc: "Correct team advancing" },
            ...
            <div className="border-t pt-4" style={{ borderColor: "#f1f5f9" }}>
              ...
              {[
                { label: "Tournament winner pick", val: tournamentWinner, set: setTournamentWinner, enabled: enableWinner, setEnabled: setEnableWinner },
                { label: "Top scorer pick",        val: topScorer,       set: setTopScorer,       enabled: enableScorer, setEnabled: setEnableScorer },
              ].map(({ label, val, set, enabled, setEnabled }) => (
                <div key={label} className="flex items-center gap-3 mb-3">
                  <button onClick={() => setEnabled(!enabled)}
                    className="h-5 w-9 rounded-full transition-all shrink-0"
                    style={{ background: enabled ? "#00D4FF" : "#e2e8f0" }}>
                    <div className="h-4 w-4 rounded-full bg-white shadow transition-all mx-auto"
                      style={{ marginLeft: enabled ? "18px" : "2px" }} />
                  </button>
                  <span className="text-sm flex-1" style={{ color: enabled ? "#0F172A" : "#94a3b8" }}>{label}</span>
                  {enabled && (
                    <div className="flex items-center gap-1.5 shrink-0">
                      <input type="number" min={0} value={val}
                        onChange={e => set(Number(e.target.value))}
