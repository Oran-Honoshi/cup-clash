...
const DEFAULT_RULES: ScoringRules = { exactScore: 25, correctOutcome: 10, tournamentWinner: 100 };
...
export function ShareGroup({ groupName, adminName: _adminName, passkey, compact = false, scoringRules, paymentLink }: ShareGroupProps) {
  ...
  const scoring = `${t("sc_exact")} ${rules.exactScore}pts · ${t("sc_outcome")} ${rules.correctOutcome}pts · ${t("sc_winner")} ${rules.tournamentWinner}pts`;

  const message = [
    `🏆 ${groupName} ${t("share_invite")}`,
    ``,
    `🔑 ${t("share_passkey")}: ${passkey}`,
  ...
  const nativeShare = async () => {
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({ title: `Join ${groupName} on Cup Clash`, text: message, url: joinUrl });
        return;
            ...
            >
              ...
              <div className="px-5 pb-8 space-y-5">
                ...
                <div className="rounded-2xl p-4 text-center"
                  ...
                  <div className="text-xs mt-1" style={{ color: "#94a3b8" }}>{t("share_join_free")}</div>
                </div>
                ...
                <div>
                  ...
                  <div className="rounded-xl p-3 mt-2 text-xs leading-relaxed"
