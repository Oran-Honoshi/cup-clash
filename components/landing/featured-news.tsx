    ...
    <section className="py-24 px-5 sm:px-8" style={{ background:"rgba(0,212,255,0.02)" }}>
      <div className="max-w-7xl mx-auto">
        <motion.div initial={{ opacity:0, y:16 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }}
          ...
          <Link href="/schedule" className="flex items-center gap-1.5 text-sm font-bold uppercase tracking-widest transition-colors"
            style={{ color:"#94a3b8" }}
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {NEWS.map((item, i) => (
            <motion.div key={item.headline}
              ...
              <Link href={item.href} className="group flex flex-col rounded-2xl overflow-hidden transition-all block h-full"
                ...
                <div className="relative h-48 overflow-hidden">
                  ...
                  <div className="absolute top-3 left-3">
                    <span className="text-[9px] font-bold uppercase tracking-widest px-2 py-1 rounded-full"
                      style={{ background:"rgba(255,255,255,0.9)", color:item.color, border:`1px solid ${item.color}30`, backdropFilter:"blur(8px)" }}>
                      {item.tag}
                    </span>
                  </div>
                </div>
