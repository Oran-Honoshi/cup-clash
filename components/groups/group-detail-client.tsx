...
const SCORING_LABELS: Record<string, string> = {
  ...
  best_third:        "Best 3rd-place (each)",
};

const ENABLE_KEYS: Record<string, string> = {
  correct_outcome:   "enable_outcome",
  exact_score:       "enable_exact",
  ko_advancement:    "enable_ko_advancement",
  tournament_winner: "enable_winner",
  top_scorer:        "enable_scorer",
  top_assister:      "enable_assister",
  golden_ball:       "enable_golden_ball",
  best_defence:      "enable_best_defence",
  best_young_player: "enable_best_young_player",
  best_third:        "enable_best_third",
};
...
export function GroupDetailClient({ group, rules, members, currentUserId, isAdmin, isMember }: GroupDetailClientProps) {
  ...
  const scoringRows = Object.entries(SCORING_LABELS)
    .filter(([key]) => {
      const enableKey = ENABLE_KEYS[key];
      return !enableKey || rules?.[enableKey] !== false;
    });
      ...
      <div className="flex items-start justify-between">
        <div>
          <Link href="/groups" className="text-xs font-bold uppercase tracking-widest mb-2 flex items-center gap-1"
            style={{ color: "#0891B2" }}>← My Groups</Link>
          <h1 className="font-display text-4xl uppercase font-black" style={{ color: "#0F172A" }}>{group.name}</h1>
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            {isAdmin  && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: "rgba(217,119,6,0.1)", color: "#d97706" }}>Admin</span>}
            {isMember && <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ background: "rgba(0,255,136,0.1)", color: "#059669" }}>Member</span>}
          </div>
        </div>
        {isAdmin && (
        ...
        {TABS.map(t => {
          ...
          return (
            <button key={t.id} onClick={() => setTab(t.id as "overview" | "chat")}
              ...
              style={active ? {
                background: "rgba(255,255,255,0.9)", border: "1px solid rgba(0,212,255,0.3)",
                color: "#0F172A", boxShadow: "0 4px 12px rgba(0,212,255,0.08)",
              } : {
                background: "rgba(255,255,255,0.5)", border: "1px solid #e2e8f0", color: "#64748b",
              }}>
              <t.icon size={15} style={{ color: active ? "#0891B2" : "#94a3b8" }} />
              {t.label}
              {t.id === "chat" && (
      ...
      {tab === "overview" && (
        <div className="space-y-5">
          ...
          <div className="rounded-2xl p-5"
            style={{ background: "rgba(255,255,255,0.9)", border: "1px solid rgba(0,212,255,0.15)" }}>
            <div className="text-xs font-bold uppercase tracking-widest mb-4" style={{ color: "#64748b" }}>Prize Split</div>
            <div className="space-y-2">
              {[
                { label: "🥇 1st", pct: group.payout_first  ?? 60, color: "#d97706" },
                { label: "🥈 2nd", pct: group.payout_second ?? 30, color: "#64748b" },
                { label: "🥉 3rd", pct: group.payout_third  ?? 10, color: "#b45309" },
              ].map(({ label, pct, color }) => (
                <div key={label} className="flex items-center gap-3">
                  <span className="text-sm w-12" style={{ color: "#475569" }}>{label}</span>
                  ...
                  {totalPot > 0 && <span className="text-xs w-12 text-right" style={{ color: "#64748b" }}>${Math.round(totalPot * pct / 100)}</span>}
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="grid grid-cols-2 gap-2">
              {scoringRows.map(([key, label]) => (
                <div key={key} className="flex items-center justify-between rounded-xl px-3 py-2.5"
                  ...
                  <span className="text-sm font-black" style={{ color: "#0891B2" }}>+{rules?.[key] as number ?? "—"}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="rounded-2xl overflow-hidden"
            style={{ background: "rgba(255,255,255,0.9)", border: "1px solid rgba(0,212,255,0.15)" }}>
            <div className="px-5 py-4 border-b flex items-center justify-between" style={{ borderColor: "rgba(0,212,255,0.1)" }}>
              <span className="text-xs font-bold uppercase tracking-widest" style={{ color: "#64748b" }}>Members ({paidCount} paid)</span>
            </div>
                    ...
                    <div className="text-sm font-bold truncate" style={{ color: "#0F172A" }}>
                      ...
                      {m.user_id === currentUserId && (
                        <span className="ml-1.5 text-[10px] font-bold px-1.5 py-0.5 rounded-full"
                          style={{ background: "rgba(0,255,136,0.1)", color: "#059669" }}>You</span>
                      )}
                    </div>
                    <div className="text-xs" style={{ color: "#94a3b8" }}>{m.profiles?.country ?? ""}</div>
                  </div>
