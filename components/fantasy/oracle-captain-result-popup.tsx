...
interface OracleCaptainResultPopupProps {
  result: LatestOracleCaptainResult;
  onDismiss: () => void;
}

// Win-celebration modal for Oracle Captain — same glass-card/spring-
// animation shell and green/red/gold grammar as OracleDuelResultPopup,
// adapted for a single captain result (Oracle right → x3, wrong → x2)
// instead of a user-vs-Oracle score comparison.
export function OracleCaptainResultPopup({ result, onDismiss }: OracleCaptainResultPopupProps) {
  const router = useRouter();

  const outcomeColor = result.won ? "#00c46a" : "#f87171";
  const outcomeTitle = result.won ? "Oracle Captain Hit!" : "Oracle Captain Missed";
      ...
      >
        ...
        >
          <div
            ...
            style={{ background: result.won ? `linear-gradient(90deg, ${GOLD}, #F5E06E)` : "linear-gradient(90deg, var(--ac), var(--ac2, var(--ac)))" }}
          />
          ...
          <div className="flex items-center gap-2 mb-3">
            {result.won && <Trophy size={18} style={{ color: GOLD }} />}
          ...
          <div className="space-y-2 mb-5">
            <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: "var(--ip)" }}>
              <div>
                <div className="text-sm font-bold" style={{ color: "var(--tx)" }}>Your Captain</div>
                <div className="text-xs" style={{ color: "var(--t2)" }}>
                  {result.captainName} — {result.basePoints} pts × {result.multiplier}
                </div>
              </div>
              <div className="font-display text-2xl font-black" style={{ color: outcomeColor }}>+{result.totalPoints}</div>
            </div>
            <div className="flex items-center justify-between p-3 rounded-xl" style={{ background: "var(--ip)" }}>
              <div>
                <div className="text-sm font-bold" style={{ color: "var(--tx)" }}>Oracle Says</div>
                <div className="text-xs" style={{ color: "var(--t2)" }}>
                  {result.oraclePredictedWinner ? `Predicted: ${result.oraclePredictedWinner}` : "No prediction on record"}
                </div>
              </div>
              <div className="font-display text-sm font-black" style={{ color: "var(--t2)" }}>
          ...
          <button
            type="button"
            onClick={() => { onDismiss(); router.push(`/fantasy/${result.fantasyLeagueId}`); }}
